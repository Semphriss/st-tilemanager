...
#include "supertux/util/writer.hpp"
  ...
  lists()
{
  ...
  if (!fout->good())
  {
    ...
    msg << "Writer problem: Couldn't open file '" << filename << "' for writing.";
...
{
  if (lists.size() > 0) {
    log_warn << m_filename << ": Not all sections closed in Writer" << std::endl;
  }
  if (out_owned)
  {
...
{
  if (lists.size() == 0) {
    log_warn << m_filename << ": Trying to close list '" << listname << "', which is not open" << std::endl;
    return;
  }
  if (lists.back() != listname) {
    log_warn << m_filename << ": trying to close list '" << listname << "' while list '" << lists.back() << "' is open" << std::endl;
    return;
  }
  lists.pop_back();
